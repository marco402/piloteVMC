...
/* ======================================================================
Function: attachPTEC 
Purpose : attach a callback when we detected a PTEC on any phase
Input   : callback function
Output  : - 
Comments: -
====================================================================== */
void ICACHE_FLASH_ATTR TInfo::attachPTEC(void (*fn_PTEC)(uint8_t phase))
{
...
Comments: -
====================================================================== */
void ICACHE_FLASH_ATTR TInfo::attachNewFrame(void(*fn_new_frame)(ValueList * valueslist))
{
	// indicate the user callback
...
{
//   int8_t etat=0;
  if (!strcmp(plabel, TableauTempoName[6]))  //PTEC
  {
	for (int etat=0;etat<6;etat++)
	{
		if (!strcmp(pvalue, tableauPtec[etat]))
		{
			// signalement PTEC a changé
			if (_fn_PTEC )
				_fn_PTEC(etat);				//transfert du nouveau compteur
			break;	
		}
	}
  }
           ...
           // customLabel(ptok, pvalue, &flags);
            ...
            if ( me ) {
              ...
              if (flags & (TINFO_FLAGS_UPDATED | TINFO_FLAGS_ADDED | TINFO_FLAGS_ALERT) ) {
                // this frame will for sure be updated
                _frame_updated = true;
				customLabel(ptok, pvalue, &flags);  //deplacé ici pour évenement PTEC que sur changement de compteur
  ...
  switch (c)  {
    ...
    case  TINFO_ETX:
      ...
      if (_state == TINFO_READY) {
        ...
        // to remove the alert flags from table (PTEC for example)
