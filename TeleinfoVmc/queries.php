...
class queries
{
    ...
    public static function queryDaily ($timestampdebut, $timestampfin, $optarif = null) {
        ...
        $mesures = array_merge($mesures, array("OPTARIF", "PTEC","ANNOTATIONS"));
    ...
    public static function queryFonctionnel ($timestampdebut, $timestampfin, $optarif = null) {
        global $db_connect, $config_table;
        global $teleinfo;


         //$mesures =array($config_table["tableoptionnel"]);

       // $mesures = array ("DUREEMAXI");
      //  $mesures[] = "ETATWIFI";
        $tDate = strtoupper($config_table["type_date"]);
        $timestamp = self::dateConvSQL($config_table["table"]["DATE"], $tDate, "TIMESTAMP");
        ...
        $query = "SELECT " . $timestamp . " AS TIMESTAMP, ";
        // Mesures
        foreach(($config_table["tablefonctionnel"])  as $key => $field) {
            $query .= $field . " AS " . $key . ", ";
        }
        $query .= "annotations AS ANNOTATIONS, ";
    ...
    public static function queryVmc($timestampdebut, $timestampfin, $optarif = null) {
        global $db_connect, $config_table;
        global $teleinfo;
        //$mesures = array ("DUREEMAXI","ETATWIFI");
        $tDate = strtoupper($config_table["type_date"]);
        $timestamp = self::dateConvSQL($config_table["table"]["DATE"], $tDate, "TIMESTAMP");
        // Select Timestamp
        $query = "SELECT " . $timestamp . " AS TIMESTAMP, ";
        // Mesures
        foreach(($config_table["tablevmc"])  as $key => $field) {
                 $query .= $field . " AS " . $key . ", ";
        }
        $query .= "annotations AS ANNOTATIONS ";
        // Suppression de la dernière virgule
    ...
    public static function queryHistory ($timestampdebut, $timestampfin, $dateformatsql, $optarif) {
        global $db_connect, $config_table;
        global $teleinfo;
        //SELECT DATE AS TIMESTAMP, DATE_FORMAT(DATE(FROM_UNIXTIME(DATE)), '%a %e') AS PERIODE,  GROUP_CONCAT(ANNOTATIONS SEPARATOR ' ')
        // AS annotations FROM pilotevmctempo WHERE DATE BETWEEN '1559167200' and '1560549600' GROUP BY PERIODE ORDER BY DATE

        //SELECT DATE AS TIMESTAMP, DATE_FORMAT(DATE(FROM_UNIXTIME(DATE)), '%a %e') AS PERIODE,
        // OPTARIF AS OPTARIF, PTEC AS PTEC, ROUND(((MAX(NULLIF(BBRHPJB, 0)) - MIN(NULLIF(BBRHPJB, 0))) / 1000), 1) AS HPJB,
        // ROUND(((MAX(NULLIF(BBRHPJW, 0)) - MIN(NULLIF(BBRHPJW, 0))) / 1000), 1) AS HPJW, ROUND(((MAX(NULLIF(BBRHPJR, 0)) - MIN(NULLIF(BBRHPJR, 0))) / 1000), 1) AS HPJR,
        // ROUND(((MAX(NULLIF(BBRHCJB, 0)) - MIN(NULLIF(BBRHCJB, 0))) / 1000), 1) AS HCJB, ROUND(((MAX(NULLIF(BBRHCJW, 0)) - MIN(NULLIF(BBRHCJW, 0))) / 1000), 1) AS HCJW, 
        //ROUND(((MAX(NULLIF(BBRHCJR, 0)) - MIN(NULLIF(BBRHCJR, 0))) / 1000), 1) AS HCJR , GROUP_CONCAT(ANNOTATIONS SEPARATOR ',') AS annotations
        // FROM pilotevmctempo WHERE DATE BETWEEN '1559167200' and '1560549600' GROUP BY PERIODE ORDER BY DATE
        ...
        $query .= "GROUP_CONCAT(ANNOTATIONS SEPARATOR ',') AS annotations ";
        // Suppression de la dernière virgule
        //$query = substr($query, 0, -2) . " ";
        // From
        $query .= "FROM " . $db_connect['table'] . " ";
