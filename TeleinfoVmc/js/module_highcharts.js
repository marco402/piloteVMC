...
var modHighCharts = (function () {
 ...
 //   }
    ...
    function init_chart0(data, serie) {
        ...
        var graphOptions = {
            chart: {
                ...
                plotShadow: false,
                events: {
                    load: function () {chart_loaded('#' + this.renderTo.id, data.subtitle); },
                    resize: function () {
                        var pane_num;
    ...
    function addannot($X,$Y)
    {
        var annotation = prompt("Nouvelle annotation(16 caractères maximum) x: " + $X + " y: " + $Y, "");
        if (!(annotation == null || annotation == "")) 
            setAnnotationToDatabase(annotation,$X/1000);
 ...
 //       $.each(data.annotation, function (x, y) {
 //           lesannotations.push({
  //               labels: [{
  //                   point: {
  //                       x : x,
  ...
  //       }
        ...
        var graphOptions = {
            chart: {
                ...
                alignTicks: false,
                zoomType: 'xy', // Si on veut activer le dblclick : http://www.highcharts.com/plugin-registry/single/15/Custom-Events
                events: {
                    load: function () {chart_loaded('#' + this.renderTo.id, data.subtitle); }
                }
            },
    ...
    function init_chart3(data) {
       // Préparation des séries de données
        ...
        // Période courante
        $.each(data.series, function (index,serie_name) {
            //if (data[serie_name + "_data"] !== undefined) {
                graphSeries.push({
                    name: data[serie_name + "_name"],             //data.series["D"] +
                    data: data[serie_name +  "_data"],
                    color: data[serie_name + "_color"],
                    id: serie_name,
                    type: 'line',
                    step: true,
                    threshold: null,
                    yAxis: data[serie_name + "_yaxis"],
                    visible:true  ,  // (data[serie_name + "_data"].reduce(function (a, b) { return a + b[1]; }, 0) !== 0),
                    showInLegend:true   // (data[serie_name + "_data"].reduce(function (a, b) { return a + b[1]; }, 0) !== 0)
                });
         });
        ...
        var graphOptions = {
            ...
            yAxis: [{ // Primary yAxis      humidité  
                opposite: true, //3 axes a droite
               // max:100,    //si <70 annotations pas visibles inversé état et humidité
               // min:0,
                title: {
                    text: 'H%'
                },
            ...
            tooltip: 
            {
             ...
             annotations: [{
                labelOptions: {
                   ...
                   // verticalAlign: 'top',       //flèche du label
                   // crop: true,
                   y: 100,   //0 en haut 100 en bas ????
                },
                labels: data.annotation,
               }], 
              plotOptions: {
    ...
    function init_chart4(data) {
        ...
        var graphOptions = {
            ...
            yAxis: [{ // Primary yAxis durée enregistrement
                opposite: true,
               // max:data.DUREEMAXI_ymax,
               // min:data.seuils.min,
                title: {
                    text: 'ms'
                },
                ...
                plotLines: [{ // lignes min et max
                    ...
                    label: {
                        ...
                        text: 'min. ' + data.seuils.min + 'ms.'
                    }
                }, {
                    value: data.DUREEMAXI_ymax,
                    yDecimals:2,
                    color: data.MAX_color,
                    dashStyle: 'shortdash',
                    width: 2,
                    label: {
                        ...
                        text: 'max. ' + data.DUREEMAXI_ymax + 'ms.'
                    }
                }]
            }, { // Secondary yAxis  etat wifi
                max:data.ETATWIFI_ymax,
                opposite: true,
                gridLineWidth: 0,
                showEmpty: false,
                title: {
            ...
            }, { // Third yAxis       durée de cycle
               // max:data.DUREECYCLESVMC_ymax,
                opposite: true,
                gridLineWidth: 0,
                showEmpty: false,
                title: {
    ...
    return {
        ...
        init_chart3: init_chart3,
        init_chart4: init_chart4
    };
}());
